...
class AAE(object):
    ...
    def build(self, L_G_type=1):
        q_z = self._build_latent_space()
        self._build_VAE_network(q_z)
        self._build_GAN_network(q_z, L_G_type)
    ...
    def _build_VAE_network(self, q_z):
        # Decoding: reconstruct images by drawing samples from q(z)
        recon_imgs = self.decoder(q_z, is_training=True)
    ...
    def _build_GAN_network(self, q_z, L_G_type):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            fake_logits = self.discriminator(q_z, self.y_labels, is_training=True)
            ...
            self.L_D = -tf.reduce_mean(tf.log(tf.sigmoid(true_logits)) + tf.log(1. - tf.sigmoid(fake_logits)))

            if L_G_type == 1:
                self.L_G = tf.reduce_mean(1. - tf.log(tf.sigmoid(fake_logits)))
            elif L_G_type == 2:
                self.L_G = -tf.reduce_mean(tf.log(tf.sigmoid(fake_logits)))
    ...
    def train_VAE(self, X, sess):
        feed_dict = {
            self.X: X
        }

        _, recon_loss = sess.run([self.vae_train_op, self.recon_loss], feed_dict=feed_dict)

        return recon_loss

    def train_GENERATOR(self, X, y, sess):
        feed_dict = {
            self.X: X,
            self.y_labels: y
        }

        _, gen_loss = sess.run([self.gen_train_op, self.L_G], feed_dict=feed_dict)

        return gen_loss

    def train_DISCRIMINATOR(self, X, y, sess):
        ...
        _, disc_loss = sess.run([self.disc_train_op, self.L_D], feed_dict=feed_dict)
